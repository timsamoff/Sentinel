...
You are auditing this project's own development history to find every place a "new component," "new feature," or "new [PROJECT-SPECIFIC: the recurring unit of work in this codebase — e.g. component type, endpoint, screen, model field, CLI subcommand, message/event type, game entity type, infrastructure resource]" has ever needed to touch — including places that were touched *late* or *after a bug was found* rather than up front. Do not rely on your general sense of good practice — ground every item in something that actually happened in this codebase, by reading its real history (commits, session notes, AGENTS.md, changelogs), not in memory of other projects or prior conversations with this user. Where you're not sure something is a real touchpoint, say so explicitly rather than guessing.

Check `PROJECT_PROFILE.md` for a "flagged for follow-up" section from the discovery prompt — anything incidental it noticed but couldn't act on (a license/manifest mismatch, a tracked-but-gitignored file) needs a real disposition now that `sentinel-notes/TODO.md` exists to hold it, not to stay stranded in a profile file nobody re-reads. Fold it into `TODO.md` as an ordinary one-line item, the same as anything else this audit finds. More generally: any finding this audit surfaces, including ones outside its primary touchpoint focus, needs a disposition — fixed, logged to `TODO.md`, or explicitly decided against with reasoning stated — not just a mention in passing that never gets tracked anywhere.
...
**"Was the README touched" is a necessary check, not a sufficient one — say so explicitly rather than treating file-level co-modification as proof of currency.** A commit can genuinely modify both the source file and the README in the same commit for entirely unrelated reasons (a docs typo fix landing in the same commit as an unrelated feature, a multi-purpose commit touching several things at once) and still leave the specific new behavior completely undocumented — the file-level check passes cleanly with a real gap sitting underneath it. This isn't hypothetical: it's exactly what happened during this suite's own development, caught only because a human asked directly, not because the file-level check caught it. Where it's practical for this project's stack, strengthen the README gate with a second layer beyond "did this file change": a maintained mapping of known features/mechanisms to an expected keyword or phrase in the README, checked against the README's *current content* rather than whether it changed at all — this catches "the new thing was never actually documented" in a way file-level diffing structurally cannot, since file-level diffing only sees *that* something changed, never *what*. This is still a presence check, not a correctness check (it can confirm a keyword exists, not that the surrounding prose is accurate or complete) — say so plainly in the check's own output rather than implying more confidence than a keyword match earns. Propose this second layer to the user as a genuine strengthening option rather than a mandatory addition; whether the added maintenance cost (a mapping that needs a new line every time a real feature is added) is worth it for this specific project's size and pace is a real tradeoff, not a foregone yes.

If `PROJECT_PROFILE.md` or `AGENTS.md` records a design document (`docs/DESIGN.md`, `docs/ARCHITECTURE.md`, or similar), add a third gate for it — with a meaningfully narrower trigger than the other two. `AGENTS.md` tracks fine-grained operational detail and should get touched often; a design document is a high-level narrative overview and should only need updating for genuinely architecturally significant changes (a new subsystem, a structural shift, a change to how major components relate), not every feature that would trigger the `AGENTS.md` gate — the same bar would make this gate noisy enough that people ignore it, defeating the point. Judgment is needed here in a way it isn't for the other two: propose what counts as "architecturally significant" for this specific project, with a couple of concrete examples of what would and wouldn't trigger it, and get the user's confirmation before finalizing — this is a subjective threshold the person running this audit likely has real opinions about, not something to decide unilaterally and announce. If no design document exists, this gate doesn't apply.

Add a fourth gate for `sentinel-notes/TODO.md` completion itself — a common failure mode is finishing the work an item describes without ever marking it done, leaving the tracker silently wrong. This splits into a hard check and a soft one, since only half is mechanically detectable: for any item linked to a `sentinel-notes/[issue]-design-brief.md` file, if a commit modifies that brief without a corresponding status change in `TODO.md`, that's a real structural signal, not a guess — block or warn on it. For plain one-line items with no linked brief, there's no equivalent signal to correlate against; don't invent a fuzzy heuristic to force a hard check where none is reliable — implement this half as a non-blocking reminder instead, the same pattern already used for things that need a nudge but can't be mechanically verified (a README-staleness reminder is a common instance of this same pattern).
...
2. **An updated gate-check step** (describe the logic in plain terms first, before writing any script) that, on each relevant commit, cross-references the diff against `INTEGRATION_CHECKLIST.md` and flags any category that wasn't touched but plausibly should have been — AGENTS.md, README, the design document (if one exists), and TODO.md completion sync as separate categories, each with the trigger described in Part 2. It must:
   - Run automatically before committing, not as a manual step someone has to remember
   - Fold in the commit conventions from Part 4 and the exception mechanism from Part 5
   - Also describe a full-repo scan mode, separate from the diff-scoped commit-time check — the commit-time check structurally can't catch drift from git history, edits outside a commit, or an interaction between two separately-valid commits. Don't run this on every commit (too slow, either discourages small commits or trains people to bypass the hook); propose a CI-scheduled job if the project has CI, or a `pre-push` hook otherwise
   - Make every flagged violation actionable: the specific rule/category, the exact file (and line, where diff/grep-based), and the required correction or self-review question — not just which category was flagged
...
Check `PROJECT_PROFILE.md` first (if present) for a detected existing commit-message convention. If the project already has a real, consistent pattern in its own history, propose matching it as the primary option when you ask, rather than presenting a blank slate of choices — matching what's already there is usually the right default for an established project. Then ask the user directly what commit message style they prefer — don't assume terse one-liners or any other specific style is the default. A short set of options to offer: whatever convention was detected (if any); terse one-line summaries, single-line subject only, no body, ever; Conventional Commits format (`feat:`, `fix:`, `chore:` prefixes); detailed narrative descriptions; or their own custom convention. If they have no preference and nothing was detected, terse subject + optional short bullets is a reasonable fallback, but say so explicitly as a fallback rather than presenting it as the norm. Also ask whether they prefer a single commit per session as the default or are comfortable with multiple atomic commits for larger changes — teams and individuals genuinely differ here.

**"No body, ever" means exactly that — it is not a soft default that yields to judgment.** A vague "no body unless truly necessary" leaves an open question every single commit — what counts as necessary is never defined, so an agent can quietly drift toward adding a body anyway whenever a particular change feels complex, even though the user picked the plain option specifically to avoid that. If a commit's actual scope is broad enough that a single-line subject genuinely cannot honestly summarize it — several unrelated changes bundled into one commit is the concrete case, not "this felt like it needed more explanation" — that's a signal the commit should have been split into multiple single-line commits instead, not a license to add a body to this one. The bullet-supported fallback option exists precisely for people who *do* want occasional detail; someone who picked plain terse chose it instead of that option, and the prompt should hold that distinction rather than blur it back together at the first commit that feels a little involved.
...
Do not implement or modify any gate-check scripts yet. Stop after producing the deliverables above (including the Part 4 conventions and Part 5 exception mechanism folded into deliverable 2) and wait for review. Tell the user plainly what the next step in the sequence is: run the Gate-Check Implementation prompt next.
