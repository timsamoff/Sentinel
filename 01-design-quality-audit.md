...
If no design document exists, ask the user whether they want one created — don't create it by default. If they say yes: write it as honest as-built documentation reverse-engineered from the current codebase, not as a traditional forward-looking design document. Say so explicitly in the document itself — an AI can describe what a system does with real confidence, but has much shakier ground for the tradeoffs and rejected alternatives that happened in someone's head, which is exactly what a genuine design document usually claims to capture. Presenting an inferred architecture with the unearned authority of deliberate rationale would be actively misleading.

Check whether `docs/` already exists and what it's actually used for before defaulting to `docs/DESIGN.md`. A `docs/` folder used for user-facing or rendered documentation (a docs site, published guides) is a different audience than an internal design document, and dropping the two in the same folder mixes them — a real, common collision, not an edge case. If `docs/` is free, unused, or doesn't exist yet, `docs/DESIGN.md` is the right default. If `docs/` is already claimed by something else, don't silently pick a workaround — ask the user, offering the repo root (`DESIGN.md` or `ARCHITECTURE.md`, an equally well-established convention sitting alongside `README.md`) as the natural fallback, or a subfolder like `docs/dev/` or `docs/internal/` if the project's own structure suggests that fits better.

Keep its scope distinct from `AGENTS.md` rather than duplicating it: `AGENTS.md` is operational context for an AI agent working in this codebase; a design document is a narrative overview for a human reader — onboarding, stakeholder communication, understanding the system's shape at a glance. If the two would just say the same things in different registers, that's a sign a second document isn't earning its keep — say so rather than creating it anyway.

Write the document itself in standard technical-documentation voice — descriptive and impersonal, never first-person. "Areas with unclear or unrecoverable rationale," not "things I was unsure about." This applies even where the document is marking its own uncertainty — honestly flagging unclear rationale is good practice and belongs in the document, but it should read like documentation noting a gap, not like a session diary. Save first-person language for your own chat-facing report to the user; it should never appear in the document's actual content.
...
Ask one more thing alongside it: once a `TODO.md` item is completed, what should the agent do next? Three genuinely different options, not two — stop and wait to be told what's next; move on to the next item in list order (positional, no judgment involved); or actively recommend a couple of candidates worth doing next, with a stated reason for each, based on their descriptions. The third option is meaningfully different from the second — "next in the list" and "worth doing" aren't the same thing, and conflating them means picking one silently answers a question the user never actually got asked. Offer a small handful of options rather than a single forced pick when using this mode — real choice, not one imposed recommendation — and where the wording plausibly signals it, include at least one candidate that reads as a quick, mechanical win (verbs like "remove," "align," "rename" versus "reconcile," "consolidate," "redesign," which read as more involved) alongside anything else worth surfacing. If the user picks the third option, note in `AGENTS.md` that `TODO.md` items are plain one-line descriptions with no priority, effort, or dependency data attached — any recommendation, including apparent quick-win signal, is reasoning from the description text and its wording alone, not from real prioritization or effort metadata, so it's a judgment call to state plainly every time, not a fact to assert. This governs ordinary day-to-day work through the backlog, not just this audit's own session — record whichever option is chosen in `AGENTS.md` the same way, since it's a standing behavioral preference, not a one-time setup choice. There's no default here; ask rather than assuming any of the three.

Record one more standing note in `AGENTS.md` alongside these, without needing to ask it as a separate question: updating `TODO.md` itself is part of finishing the work an item describes, not a follow-up step to remember afterward — the same way a test passing or a linked design brief being resolved is part of being done. This is worth stating explicitly because it's easy to update the actual code and a linked design brief while forgetting the tracker that's supposed to reflect both, and instruction alone won't reliably prevent that on its own — the Integration Audit's gate check backs this up mechanically wherever it can (see Part 2), but this note is the softer layer that still applies to the cases the mechanical check can't reach.
...
- Within `./sentinel-notes/`, create a `TODO.md` listing discovered action items as a plain list, noting the completed-item convention and next-item-behavior just confirmed at the top of the file. Every entry is exactly one line: a checkbox, a short rule-ID or title tag if relevant, and a single-sentence description — never more than that in the file itself. This is a hard rule, not a target to aim for: if what you'd write about an item takes more than one sentence, involves multiple sub-points, lists specific files/lines, or explains *why* something is the way it is rather than just *what* it is, that's design-brief territory — write `./sentinel-notes/[issue]-design-brief.md` instead and have the `TODO.md` entry link to it. This rule holds every time `TODO.md` is written to by any of these prompts, not just here at creation — the Integration Audit and Gate-Check Implementation prompts both write to this file later, and both are expected to follow this same one-line-or-spin-off-a-brief standard, not their own separate judgment about what counts as brief.
