...
Ask one more thing alongside it: once a `TODO.md` item is completed, should the agent proactively suggest moving on to the next one, or stop and wait to be told what's next? This governs ordinary day-to-day work through the backlog, not just this audit's own session — record it in `AGENTS.md` the same way, since it's a standing behavioral preference, not a one-time setup choice. There's no default here either; some people want momentum through a backlog, others find it presumptuous — ask rather than assuming either.
