...
- **Git is the default, not the only option.** Prompts 2 and 3 both handle no-VCS projects (2 skips commit-message conventions, 3 builds a manual command instead of a hook, with a clear note that nothing will remind you to run it). A different VCS with its own real trigger mechanism (Perforce's triggers, common in game dev) gets treated the same as a git hook — the manual-command fallback is only for projects with no trigger mechanism at all. The same manual-command path also covers a narrower case even *with* git in use: if some of Sentinel's own output (`AGENTS.md`, a design doc, `TODO.md`) ends up gitignored per your own local-vs-shared choice, no hook can ever see those files to check them — hooks only see staged/committed content — so those specific checks route to a working-tree-reading script instead, regardless of whether hooks are used for everything else.
