...
- **Run prompt 0 first if you can.** It fills in the brackets for you with a reviewable record of what it inferred. Skipping it isn't a failure mode — your agent will typically infer reasonable values — just less reliable than a deliberate, recorded answer.
...
- `sentinel-notes/` — audit findings and a deliberately brief `TODO.md`: one line per item, anything needing more detail split into its own linked design brief.
