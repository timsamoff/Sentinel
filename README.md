...
- **The orchestrator moves between stages on its own, by default.** `sentinel-init.md` states its recommended next step and proceeds, rather than stopping to ask "run the next one?" at every stage boundary — you can redirect it at any point. The one exception: it still genuinely stops and waits before prompt 3 implements anything, confirming you've actually reviewed what 1 and 2 proposed first. Every stage's own internal questions (design-doc creation, comment philosophy, commit conventions, and the rest) still stop and wait too — this default only covers whether to advance to the next stage, not what happens inside one.
- **Choices render as real, clickable options when your tool supports it.** Where a question has a fixed set of answers, the prompts ask your agent to use a structured choice/selection mechanism if one's available, rather than always falling back to typed free-text replies.
- **The plain terse commit option means no body, ever — not "no body unless truly necessary."** That vague wording used to leave room for an agent to add one anyway once a change felt complex enough, even for people who deliberately chose the plain option over the bullet-supported one. Enforced as a real hard block by prompt 3's hook, not just an instruction. If a change is genuinely too broad for one honest subject line, the fix is splitting it into multiple commits — not a body on this one.
