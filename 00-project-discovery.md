...
End with a short "how to use this" note: when running the Design & Code Quality Audit or Integration Audit prompts, replace their `[PROJECT-SPECIFIC]` brackets using the corresponding values above rather than guessing fresh. Also tell the user plainly what the next step is — but this isn't the same instruction for a fresh run versus an update run. On a fresh run, that's simply "run the Design & Code Quality Audit prompt next." On an update run, be specific about *why*, not just restate the generic fresh-run guidance: if a gate-check hook was flagged for re-validation, say plainly that the Gate-Check Implementation prompt needs to run again for that reason specifically, and name which of the four prompts are actually worth re-running given what changed rather than defaulting to "start over from 01." An update run that ends without pointing at the specific follow-up it just generated has left the most important part of its own output for the user to notice on their own.
