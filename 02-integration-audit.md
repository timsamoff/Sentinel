...
`CLAUDE.md` is this project's authoritative internal record of features, conventions, and architecture — it's what future CC sessions rely on for accurate context. For every commit that adds or changes a [PROJECT-SPECIFIC unit], feature, or architectural decision, the gate check should verify `CLAUDE.md` was touched in the same commit — and if it wasn't, block or warn. This is the higher-priority of the two gates, since drift here corrupts every future session's and every future audit's starting context, not just a human reader's understanding.
